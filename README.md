...
# Remote (Supabase на Dockploy) — по умолчанию
python scripts/support_db.py health
python scripts/support_db.py metrics

# Local Docker
python scripts/support_db.py --local health
python scripts/support_db.py --local metrics
...
```bash
# Remote (Supabase REST API) — по умолчанию
...
# Local Docker (psql)
python scripts/support_db.py --local health
python scripts/support_db.py --local order ORD-1042
python scripts/support_db.py --local sql "SELECT status, count(*) FROM orders GROUP BY status"
```

**Remote (REST API):** Использует `supabase-py` для запросов через REST API.
Требуется `SUPABASE_URL` и `SUPABASE_KEY` в `.env`.

**Local (psql):** Использует `docker compose exec` для прямых SQL-запросов.
Команда `sql` доступна только с флагом `--local`.

Безопасность: команда `sql` принимает только `SELECT` и `WITH`.
...
- `supabase` — REST API клиент для Dockploy/Cloud
- `psycopg2-binary` — для `init_db.py` (прямое подключение к PostgreSQL)
